...
public class SignUpVerifyController {
    ...
    @PatchMapping("/driver-license")
    public ResponseEntity verifyDriverLicense(@AuthenticationPrincipal User principal, @RequestBody DriverAuthDto driverAuthDto) {
        memberService.verifyDriverLicense(principal.getUsername(), driverAuthDto);
        return ResponseEntity.noContent().build();
    }
